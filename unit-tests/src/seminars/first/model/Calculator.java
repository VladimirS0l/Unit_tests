...
public class Calculator {
    ...
    public static double calculatingDiscount(double purchaseAmount, double discountAmount) {
        if (checkNegativeNumbers(purchaseAmount)&&checkNegativeNumbers(discountAmount)){
            throw new ArithmeticException("Numbers negative");
        }
        if (discountAmount > 100){
            throw new ArithmeticException("Discount > 100");
        }
        return purchaseAmount * (1 - (discountAmount/100));
    }

    public static boolean checkNegativeNumbers(double number){
        if (number < 0){
            return true;
        }else {
            return false;
